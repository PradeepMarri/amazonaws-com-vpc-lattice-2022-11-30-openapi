...
func UpdatelistenerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.UpdateListenerResponse
...
func CreateUpdatelistenerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_services_serviceIdentifier_listeners_listenerIdentifier",
		mcp.WithDescription("Updates the specified listener for the specified service."),
		...
		mcp.WithObject("defaultAction", mcp.Required(), mcp.Description("Input parameter: Describes the action for a rule. Each rule must include exactly one of the following types of actions: <code>forward </code>or <code>fixed-response</code>, and it must be the last action to be performed.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatelistenerHandler(cfg),
	}
}
