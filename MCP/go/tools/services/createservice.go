...
func CreateserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/services", cfg.BaseURL)
...
func CreateCreateserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_services",
		...
		mcp.WithString("certificateArn", mcp.Description("Input parameter: The Amazon Resource Name (ARN) of the certificate.")),
		...
		mcp.WithString("customDomainName", mcp.Description("Input parameter: The custom domain name of the service.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the service. The name must be unique within the account. The valid characters are a-z, 0-9, and hyphens (-). You can't use a hyphen as the first or last character, or immediately after another hyphen.")),
		mcp.WithObject("tags", mcp.Description("Input parameter: The tags for the service.")),
	)
