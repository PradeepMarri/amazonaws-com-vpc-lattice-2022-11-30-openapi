...
func DeletelistenerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletelistenerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_services_serviceIdentifier_listeners_listenerIdentifier",
		mcp.WithDescription("Deletes the specified listener."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletelistenerHandler(cfg),
	}
}
